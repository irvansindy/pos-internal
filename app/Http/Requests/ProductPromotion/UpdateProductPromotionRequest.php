...
class UpdateProductPromotionRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'triggers.*.product_id' => ['required', 'integer', Rule::exists('products', 'id')->where('team_id', $teamId)],
            'triggers.*.min_quantity' => ['required', 'integer', 'min:1'],

            'rewards' => ['required', 'array', 'min:1'],
            'rewards.*.product_id' => ['required', 'integer', Rule::exists('products', 'id')->where('team_id', $teamId)],
